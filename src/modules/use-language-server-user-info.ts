...
type State = {
  // email -> 用户状态（使用 email 作为 key 更稳定）
  users: Record<string, LanguageServerResponse.Root>
}

type Actions = {
  fetchData: (antigravityAccount: AntigravityAccount) => Promise<boolean>
}

export const useLanguageServerUserInfo = create<State & Actions>()(
  persist(
    (setState, getState) => ({
      users: {},
      fetchData: async (antigravityAccount: AntigravityAccount) => {
        ...
        try {
          ...
          console.log('[LanguageServerUserInfo] getUserStatus response:', {
            ...
            responseEmail: data?.userStatus?.email,
          ...
          // 校验返回的数据是否属于当前请求的用户
          if (data?.userStatus?.email && data.userStatus.email !== antigravityAccount.email) {
            console.warn(`[LanguageServerUserInfo] Email mismatch! Requested: ${antigravityAccount.email}, Received: ${data.userStatus.email}. Ignoring data.`);
            return false;
          }
          ...
          setState({
            users: {
              ...getState().users,
              [antigravityAccount.email]: data
            }
          })
          console.log('[LanguageServerUserInfo] Successfully stored data for user:', antigravityAccount.email);
          console.log('[LanguageServerUserInfo] All cached emails:', Object.keys(getState().users));
          return true;
        } catch (error) {
    ...
    {
      name: 'language-server-user-info-storage', // localStorage key
      version: 1,
    }
  )
)
